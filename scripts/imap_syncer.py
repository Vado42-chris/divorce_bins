...
def sync_emails_to_vault(host, user, password, limit=50):
    ...
    for i in ids[-limit:]: # Just last N for safety in testing
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            ...
            f.write("type: email\n")
            f.write(f"source: imap://{host}/{user}\n")
            f.write(f"pristine_reference: imap_message_id_{i.decode()}\n")
            f.write("status: pending_decision\n")
            f.write("---\n\n")
            f.write(f"# Email: {subject}\n\n")
            f.write(f"**From:** {sender}\n")
            f.write(f"**Date:** {date}\n")
            f.write(f"**Sovereign Source:** {host} [READ-ONLY]\n")
            f.write(f"**Pristine ID:** {i.decode()}\n\n")
            f.write("---\n\n")
